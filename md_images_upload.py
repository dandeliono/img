...
## 用户名
user_name = "dandeliono"
## 仓库名
github_repository = "img"

## git仓库在本机的位置
git_repository_folder = "/Documents/GitHub/hexo/img"

## 存放图片的git文件夹路径
git_images_folder = "/Documents/GitHub/hexo/img/resource"
...
def get_http_image(image_url):
    ...
    proxies = { "http": "http://127.0.0.1:52019", "https": "http://127.0.0.1:52019"} 
    image_data = requests.get(image_url, headers=headers, proxies=proxies).content
    file_uuid_name = hashlib.md5(image_data).hexdigest()
    # 创建临时文件
    ...
    new_image_path_and_name = new_image_path_and_name.replace("\\","/")
    # 重命名图片
    os.rename(tmp_new_image_path_and_name, new_image_path_and_name)

    new_image_url = "https://raw.githubusercontent.com/"+ user_name + "/" +github_repository+"/main/"+git_images_folder.split("/")[-1]+"/"+new_image_path_and_name.split("/")[-1]
    image_info = {
        "image_url": image_url,
...
def get_local_image(image_url):
    ...
    try:
        ...
        with open(image_url, 'rb') as fp:
            data = fp.read()
        file_uuid_name = hashlib.md5(data).hexdigest()
        # 获取图片类型
    ...
    with open(md_file, "r+", encoding="utf8") as f:
        ...
        for image_url in image_urls:
            ...
            # 处理网络图片
                ...
                # 不爬取svg
                if(image_url.startswith("https://img.shields.io") == False and image_url.endswith(".svg") == False):
                    try:
